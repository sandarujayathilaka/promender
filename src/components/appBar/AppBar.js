...
const buttons = [
  ...
  { name: 'Get Registered', path: '/register' },
    ...
    <>
      <AppBar position="static" style={{ background: 'white' }}>
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          {isMobile && (
            <IconButton edge="start" color="" aria-label="menu" onClick={toggleDrawer(true)}>
              <MenuIcon />
            </IconButton>
          )}
              ...
              {buttons.map(({ name, path, padding }) => (
                <Button
                  ...
                  style={{
                    ...
                    fontWeight: 'bold',
                    padding,
                    background: activeButton === name ? 'linear-gradient(to right, #c51383, #653592)' : 'white',
        ...
        <Drawer anchor="left" open={isDrawerOpen} onClose={toggleDrawer(false)}>
          <List>
            {buttons.map(({ name, path }) => (
              <ListItem button key={name} component={Link} to={path} onClick={() => handleButtonClick(name)}>
